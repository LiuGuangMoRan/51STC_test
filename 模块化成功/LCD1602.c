...
sbit LCD_RS = P2^6;				//	RS:	数据/指令端，1为数据，0为指令
sbit LCD_RW = P2^5;				//	RW:	读写/写端，1为读，0为写
sbit LCD_E  = P2^7;				//	E：	使1数据有效，下降沿执行命令

#define LCD_DataPort P0		////数据输入端，传16进制数，0000 0000 
...
void LCD_ShowSignedNum(unsigned char Line,unsigned char Column,int Number,unsigned char Length)
{
	...
	for(i=Length;i>0;i--)
	{
		//这里计算的十进制数，要展示数字要对应表的数值，所以要加'0'
		LCD_WriteData(Number1/LCD_Pow(10,i-1)%10+'0');	//得到整数的每个数字
	}
}
